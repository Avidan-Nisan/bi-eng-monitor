looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    var allOptions = { models: [], dashboards: [], explores: [], views: [], tables: [] };
    ...
    function getAvailableOptions() {
      var rows = getFilteredRows(), o = { models: {}, dashboards: {}, explores: {}, views: {}, tables: {} };
      rows.forEach(function(r) { if (r.model) o.models[r.model] = true; if (r.dashboard) o.dashboards[r.dashboard] = true; if (r.explore) o.explores[r.explore] = true; if (r.view) o.views[r.view] = true; if (r.table) o.tables[r.table] = true; });
      return { models: Object.keys(o.models).sort(), dashboards: Object.keys(o.dashboards).sort(), explores: Object.keys(o.explores).sort(), views: Object.keys(o.views).sort(), tables: Object.keys(o.tables).sort() };
    }
    
    var tables = {}, views = {}, explores = {}, dashboards = {}, viewToTables = {}, viewToViews = {}, exploreToViews = {}, dashToExplores = {}, viewModels = {};
    
    function buildEntities() {
      tables = {}; views = {}; explores = {}; dashboards = {}; viewToTables = {}; viewToViews = {}; exploreToViews = {}; dashToExplores = {}; viewModels = {};
      var fr = getFilteredRows();
      fr.forEach(function(r) {
        if (r.table && !tables[r.table]) tables[r.table] = { id: 't_' + r.table, name: r.table, type: 'table', sources: [], fields: [] };
        if (r.view && !views[r.view]) views[r.view] = { id: 'v_' + r.view, name: r.view, type: 'view', sources: [], fields: [], model: null };
        if (r.explore && !explores[r.explore]) explores[r.explore] = { id: 'e_' + r.explore, name: r.explore, type: 'explore', sources: [], fields: [], model: r.model };
        if (r.dashboard && !dashboards[r.dashboard]) dashboards[r.dashboard] = { id: 'd_' + r.dashboard, name: r.dashboard, type: 'dashboard', sources: [], fields: [] };
        if (r.extendedView && !views[r.extendedView]) views[r.extendedView] = { id: 'v_' + r.extendedView, name: r.extendedView, type: 'view', sources: [], fields: [], model: null };
        if (r.includedView && !views[r.includedView]) views[r.includedView] = { id: 'v_' + r.includedView, name: r.includedView, type: 'view', sources: [], fields: [], model: null };
      });
      fr.forEach(function(r) {
      ...
      Object.keys(views).forEach(function(vw) { if (viewModels[vw]) views[vw].model = Object.keys(viewModels[vw]).join(', '); });
    ...
    function getAllEntities() { return Object.values(tables).concat(Object.values(views)).concat(Object.values(explores)).concat(Object.values(dashboards)); }
    ...
    function renderDuplicatesTab() {
      var vc = Object.values(views).filter(function(v) { return v.fields && v.fields.length >= 5; }).length, metrics = null;
      ...
      var h = '<div style="padding:10px 16px;border-bottom:1px solid #1e293b;display:flex;align-items:center;justify-content:space-between;"><div style="color:#94a3b8;font-size:11px;">Analyzing <span style="color:#e2e8f0;font-weight:500;">' + vc + '</span> views</div>';
    ...
    function getSearchMatches() {
      var ae = getAllEntities(), terms = searchTags.slice(); if (searchTerm.trim()) terms.push(searchTerm.trim()); if (terms.length === 0) return [];
      var matches = [], partial = [];
      ae.forEach(function(e) { var mc = 0, fm = []; terms.forEach(function(t) { var matched = false; if (smartMatch(e.name, t)) matched = true; if (e.fields) e.fields.forEach(function(f) { if (smartMatch(f, t)) { matched = true; if (fm.indexOf(f) === -1) fm.push(f); } }); if (matched) mc++; }); var d = { entity: e, fieldMatches: fm, totalTerms: terms.length, matchedTermsCount: mc }; if (mc === terms.length) matches.push(d); else if (mc > 0) partial.push(d); });
    ...
    function renderLineageTab() {
      var ae = getAllEntities(), sm = getSearchMatches(); highlightedEntities = sm.map(function(m) { return m.entity.id; }); var visible = ae, mode = '';
      ...
      else if (searchTags.length > 0 || searchTerm.trim()) { mode = 'search'; visible = highlightedEntities.length > 0 ? ae.filter(function(e) { return highlightedEntities.indexOf(e.id) !== -1; }) : []; }
      ...
      var edges = ''; visible.forEach(function(e) { (e.sources||[]).forEach(function(s) { var f = pos[s], t = pos[e.id]; if (!f || !t) return; var stroke = '#334155', op = 0.25, sw = 1.5; if (selectedNode) { if (s === selectedNode.id || downstream.indexOf(e.id) !== -1) { stroke = '#f97316'; op = 0.9; sw = 2; } else if (e.id === selectedNode.id || upstream.indexOf(s) !== -1) { stroke = '#06b6d4'; op = 0.9; sw = 2; } } else if (mode === 'search') { stroke = '#10b981'; op = 0.6; sw = 1.5; } var x1 = f.x + nodeW, y1 = f.y + nodeH/2, x2 = t.x, y2 = t.y + nodeH/2, mx = (x1+x2)/2; edges += '<path d="M' + x1 + ' ' + y1 + ' C' + mx + ' ' + y1 + ',' + mx + ' ' + y2 + ',' + x2 + ' ' + y2 + '" fill="none" stroke="' + stroke + '" stroke-width="' + sw + '" stroke-opacity="' + op + '"/>'; }); });
      var nodes = ''; visible.forEach(function(entity) { var p = pos[entity.id], cfg = typeConfig[entity.type], isSel = selectedNode && selectedNode.id === entity.id, isUp = selectedNode && upstream.indexOf(entity.id) !== -1, isDown = selectedNode && downstream.indexOf(entity.id) !== -1, isMatch = highlightedEntities.indexOf(entity.id) !== -1 && !selectedNode; var bc = cfg.color, bw = 1; if (isSel) { bc = '#ffffff'; bw = 2; } else if (isUp) { bc = '#06b6d4'; bw = 2; } else if (isDown) { bc = '#f97316'; bw = 2; } else if (isMatch) { bc = '#10b981'; bw = 2; } var nm = entity.name.length > 22 ? entity.name.substring(0,20) + '...' : entity.name; nodes += '<g class="node" data-id="' + entity.id + '" style="cursor:pointer;" transform="translate(' + p.x + ',' + p.y + ')"><rect width="' + nodeW + '" height="' + nodeH + '" rx="6" fill="#0f172a" stroke="' + bc + '" stroke-width="' + bw + '"/><rect x="1" y="1" width="28" height="' + (nodeH-2) + '" rx="5" fill="' + cfg.color + '" fill-opacity="0.15"/><g transform="translate(8,' + (nodeH/2-6) + ')" fill="' + cfg.color + '">' + typeIcons[entity.type] + '</g><text x="34" y="' + (nodeH/2+3) + '" fill="#e2e8f0" font-size="9" font-weight="500">' + nm + '</text></g>'; });
      ...
      var stats = ''; if (selectedNode) { stats = '<span style="color:' + typeConfig[selectedNode.type].color + ';">' + selectedNode.name + '</span> <span style="color:#06b6d4;">↑' + upstream.length + '</span> <span style="color:#f97316;">↓' + downstream.length + '</span>'; } else if ((searchTags.length > 0 || searchTerm.trim()) && highlightedEntities.length > 0) { stats = '<span style="color:#10b981;">' + highlightedEntities.length + ' matches</span>'; } else if (searchTags.length > 0 || searchTerm.trim()) { stats = '<span style="color:#ef4444;">No matches</span>'; } else { stats = '<span style="color:#64748b;">Click node to trace lineage</span>'; }
      ...
      if (mode === 'search' && visible.length === 0) { r += '<div style="padding:16px;background:#1e293b;margin:8px;border-radius:6px;"><div style="color:#f59e0b;font-size:12px;margin-bottom:12px;">No matches</div>'; if (window._partialMatches && window._partialMatches.length > 0) { r += '<div style="color:#94a3b8;font-size:10px;margin-bottom:6px;">Partial:</div>'; window._partialMatches.slice(0, 3).forEach(function(pm) { var cfg = typeConfig[pm.entity.type]; r += '<div style="padding:6px 8px;background:#0f172a;border-radius:4px;border-left:2px solid ' + cfg.color + ';margin-bottom:4px;"><div style="color:#e2e8f0;font-size:10px;">' + pm.entity.name + '</div></div>'; }); } r += '</div>'; }
      else { r += '<div style="padding:8px;overflow:auto;"><svg width="' + svgW + '" height="' + svgH + '" style="font-family:system-ui,sans-serif;">' + hdr + edges + nodes + '</svg></div>'; }
      return r + '</div>';
    }
    
    function attachEvents() {
      var ae = getAllEntities();
      container.querySelectorAll('.node').forEach(function(n) { n.addEventListener('click', function() { var id = n.dataset.id, entity = ae.find(function(x) { return x.id === id; }); if (selectedNode && selectedNode.id === id) { selectedNode = null; upstream = []; downstream = []; } else { selectedNode = entity; searchTerm = ''; searchTags = []; } render(); }); });
      ...
      container.querySelectorAll('.filter-trigger').forEach(function(t) { t.addEventListener('click', function(e) { e.stopPropagation(); var key = t.dataset.filter; openDropdown = openDropdown === key ? null : key; filterSearch[key] = ''; render(); setTimeout(function() { var inp = container.querySelector('.filter-search-input[data-filter="' + key + '"]'); if (inp) inp.focus(); }, 50); }); });
    ...
    function render() {
      ...
      var cb = container.querySelector('#clearAll'); if (cb) cb.addEventListener('click', function() { searchTerm = ''; searchTags = []; selectedNode = null; render(); });
      ...
      container.querySelectorAll('.tab-btn').forEach(function(btn) { btn.addEventListener('click', function() { activeTab = btn.dataset.tab; render(); if (activeTab === 'duplicates' && !similarResults && !analysisLoading) setTimeout(runAnalysis, 100); }); });
      attachEvents();
    }
