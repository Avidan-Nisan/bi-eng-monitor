looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    function tokenize(str) { 
      ...
      return tokens.concat(camelSplit).filter(function(v, i, a) { return a.indexOf(v) === i && v.length > 0; }); 
    }
    ...
    function expandWithSynonyms(term) { 
      var expanded = [term], termLower = term.toLowerCase(); 
    ...
    function calculateMatchScore(text, searchTerm) {
      ...
      textTokens.forEach(function(txtt) { 
        termTokens.forEach(function(tt) { 
          if (tt.length >= 3) { 
            var dist = levenshtein(txtt, tt), maxLen = Math.max(txtt.length, tt.length); 
