  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    data.forEach(function(row) {
      ...
      if (seenKeys[key]) {
        ...
        rf.forEach(function(f) { if (existing.fields.indexOf(f)===-1) existing.fields.push(f); });
      } else {
        seenKeys[key] = r;
        allRows.push(r);
      }
    ...
    function buildEntities() {
      ...
      allRows.forEach(function(r) {
        ...
        if (r.dashboard && !dashs[r.dashboard]) dashs[r.dashboard]={id:'d_'+r.dashboard,name:r.dashboard,type:'dashboard',sources:[],fields:[]};
    ...
    function renderTruncationWarning() {
      ...
      return '<div id="truncation-warning" style="margin:8px 16px;padding:10px 14px;background:#78350f20;border:1px solid #f59e0b40;border-radius:8px;display:flex;align-items:flex-start;gap:10px;">' +
        ...
        '<div style="flex:1;">' +
          '<div style="color:#fbbf24;font-size:12px;font-weight:600;margin-bottom:4px;">Data Truncated — Showing ' + totalRows + ' of ~13k rows</div>' +
          '<div style="color:#d97706;font-size:11px;line-height:1.5;">Use <strong>Looker dashboard filters</strong> (Model, Explore, etc.) to narrow data before the query runs so all relevant rows are included.</div>' +
        '</div>' +
    ...
    function attachEvents() {
      ...
      container.querySelectorAll('.node').forEach(function(n){n.addEventListener('click',function(){var id=n.dataset.id,entity=ae.find(function(x){return x.id===id;});if(selectedNode&&selectedNode.id===id){selectedNode=null;upstream=[];downstream=[];}else{selectedNode=entity;}render();});});
    ...
    function render() {
      ...
      // Header with tabs and row count
      ...
      h+='</div>';
      h+='<div style="font-size:11px;color:#64748b;">'+totalRows+' rows'+(allRows.length!==totalRows?' → '+allRows.length+' unique':'')+'</div>';
      h+='</div>';
