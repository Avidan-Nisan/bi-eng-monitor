looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    function tokenize(str) { if (!str) return []; return str.toLowerCase().split(/[_\-\s\.]+/).filter(function(t) { return t.length > 0; }); }
    
    var synonyms = { 'campaign': ['cmpgn', 'camp', 'cmp'], 'name': ['nm', 'title', 'label'], 'date': ['dt', 'time', 'timestamp', 'ts'], 'number': ['num', 'no', 'nr', 'count', 'cnt'], 'amount': ['amt', 'value', 'val'], 'customer': ['cust', 'client', 'user'], 'product': ['prod', 'item', 'sku'], 'order': ['ord', 'purchase'], 'identifier': ['id', 'key', 'code'], 'revenue': ['rev', 'sales'], 'gross': ['total', 'sum'] };
    ...
    function smartMatch(text, searchTerm, returnScore) {
      if (!text || !searchTerm) return returnScore ? 0 : false;
      ...
      else if (textNorm === termNorm) score = 98;
      else if (textLower.indexOf(termLower) !== -1 || textNorm.indexOf(termNorm) !== -1) score = 90;
      else {
        var textTokens = tokenize(text), termTokens = tokenize(searchTerm);
        if (textTokens.some(function(t) { return t.indexOf(termNorm) === 0 || termNorm.indexOf(t) === 0; })) score = 85;
        else if (termTokens.length > 0 && termTokens.every(function(tt) { return textTokens.some(function(txtt) { return txtt.indexOf(tt) !== -1 || tt.indexOf(txtt) !== -1; }); })) score = 80;
        else {
          var expandedTerms = []; termTokens.forEach(function(tt) { expandedTerms = expandedTerms.concat(expandWithSynonyms(tt)); });
          if (expandedTerms.some(function(et) { var etNorm = normalize(et); return textNorm.indexOf(etNorm) !== -1; })) score = 75;
        }
      }
      return returnScore ? score : score >= 35;
    }
    ...
    function runAnalysis() {
      ...
      setTimeout(function() {
        try {
          ...
          var viewsList = Object.values(views).filter(function(v) { return v.sqlTables && v.sqlTables.length > 0; });
          for (var i = 0; i < viewsList.length; i++) {
            for (var j = i + 1; j < viewsList.length; j++) {
              ...
              if (sharedTables.length >= 1 && similarity >= 20) {
                results.push({ v1: v1.name, v2: v2.name, type: 'view', sharedTables: sharedTables, similarity: similarity });
              }
            }
          }
          
          var exploresList = Object.values(explores).filter(function(e) { return e.sqlTables && e.sqlTables.length > 0; });
          for (var i = 0; i < exploresList.length; i++) {
            for (var j = i + 1; j < exploresList.length; j++) {
              var e1 = exploresList[i], e2 = exploresList[j];
              var sharedTables = e1.sqlTables.filter(function(t) { return e2.sqlTables.indexOf(t) !== -1; });
              var minTables = Math.min(e1.sqlTables.length, e2.sqlTables.length);
              var similarity = minTables > 0 ? Math.round((sharedTables.length / minTables) * 100) : 0;
              if (sharedTables.length >= 1 && similarity >= 20) {
                results.push({ v1: e1.name, v2: e2.name, type: 'explore', sharedTables: sharedTables, similarity: similarity });
              }
            }
          }
    ...
    function renderDuplicatesTab() {
      ...
      var html = '<div style="padding:20px 24px;border-bottom:1px solid #1e293b;"><div style="display:flex;justify-content:space-between;align-items:center;"><div><div style="color:#e2e8f0;font-size:14px;font-weight:500;">Similar Views Analysis</div><div style="color:#64748b;font-size:12px;margin-top:4px;">Comparing '+viewsCount+' views and '+exploresCount+' explores by shared SQL tables</div></div>'+(similarResults ? '<button id="refreshBtn" style="background:#8b5cf6;border:none;padding:8px 16px;border-radius:8px;color:white;font-weight:500;cursor:pointer;font-size:11px;">🔄 Refresh</button>' : '')+'</div></div>';
      
      if (analysisLoading) {
        html += '<div style="text-align:center;padding:60px 40px;"><div style="font-size:48px;margin-bottom:16px;">⏳</div><div style="color:#e2e8f0;font-size:16px;">Analyzing...</div></div>';
      } else if (analysisError) {
        html += '<div style="text-align:center;padding:60px 40px;"><div style="font-size:48px;margin-bottom:16px;">⚠️</div><div style="color:#ef4444;font-size:16px;">'+analysisError+'</div></div>';
      } else if (!similarResults) {
        html += '<div style="text-align:center;padding:60px 40px;"><div style="font-size:48px;margin-bottom:16px;">🔍</div><div style="color:#e2e8f0;font-size:16px;">Starting analysis...</div></div>';
      } else if (similarResults.length === 0) {
        html += '<div style="text-align:center;padding:60px 40px;"><div style="font-size:48px;margin-bottom:16px;">✅</div><div style="color:#e2e8f0;font-size:16px;">No Similar Views Found</div><div style="color:#64748b;font-size:13px;margin-top:8px;">No views share 20%+ of their SQL tables</div></div>';
      } else {
        html += '<div style="padding:12px 24px;border-bottom:1px solid #1e293b;background:#8b5cf615;font-size:13px;color:#a78bfa;">Found '+similarResults.length+' similar pair(s)</div><div style="max-height:400px;overflow-y:auto;">';
        similarResults.forEach(function(pair, idx) {
          var isExp = expandedDuplicates[idx];
          var color = pair.type === 'view' ? '#8b5cf6' : '#ec4899';
          html += '<div class="dup-row" data-idx="'+idx+'" style="border-bottom:1px solid #1e293b;">';
          html += '<div class="dup-header" style="display:flex;align-items:center;gap:12px;padding:14px 16px;cursor:pointer;">';
          html += '<span style="color:#64748b;">'+(isExp ? icons.chevronDown : icons.chevronRight)+'</span>';
          html += '<div style="flex:1;display:flex;align-items:center;gap:12px;">';
          html += '<div style="flex:1;"><div style="color:'+color+';font-size:13px;">'+pair.v1+'</div><div style="font-size:10px;color:#64748b;">'+pair.type.toUpperCase()+'</div></div>';
          html += '<div style="color:#64748b;font-size:20px;">↔</div>';
          html += '<div style="flex:1;"><div style="color:'+color+';font-size:13px;">'+pair.v2+'</div><div style="font-size:10px;color:#64748b;">'+pair.type.toUpperCase()+'</div></div>';
          html += '</div>';
          html += '<div style="text-align:center;"><div style="background:#8b5cf622;border:1px solid #8b5cf6;padding:4px 10px;border-radius:12px;font-size:11px;color:#a78bfa;font-weight:600;">'+pair.similarity+'%</div><div style="font-size:9px;color:#64748b;margin-top:2px;">'+pair.sharedTables.length+' shared</div></div>';
          html += '</div>';
          if (isExp) {
            html += '<div style="padding:12px 16px 16px 44px;background:#0c1222;"><div style="font-size:11px;color:#64748b;margin-bottom:8px;">Shared SQL Tables:</div><div style="display:flex;flex-wrap:wrap;gap:6px;">';
            pair.sharedTables.forEach(function(t) {
              html += '<span style="background:#06b6d422;border:1px solid #06b6d4;padding:4px 10px;border-radius:6px;font-size:11px;color:#22d3ee;">'+t+'</span>';
            });
            html += '</div></div>';
          }
          html += '</div>';
    ...
    function renderLineageTab() {
      ...
      ['table','view','explore','dashboard'].forEach(function(type) { 
        ...
        hdrHtml += '<text x="'+(colX[type]+nodeW/2)+'" y="22" text-anchor="middle" fill="'+cfg.color+'" font-size="9" font-weight="600">'+cfg.label.toUpperCase()+'</text><text x="'+(colX[type]+nodeW/2)+'" y="36" text-anchor="middle" fill="#475569" font-size="8">'+byType[type].length+' items</text>'; 
      });
