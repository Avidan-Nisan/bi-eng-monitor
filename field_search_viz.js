looker.plugins.visualizations.add({
  ...
  create: function(element, config) {
    ...
    element.innerHTML = '<div id="search-container" style="width:100%;min-height:600px;height:100%;overflow:auto;font-family:system-ui,sans-serif;background:#0f172a;"></div>';
  },
  
  updateAsync: function(data, element, config, queryResponse, details, done) {
    var container = element.querySelector('#search-container');
    
    console.log('Field Search Viz - Starting render');
    console.log('Field Search Viz - Data:', data ? data.length + ' rows' : 'no data');
    
    if (!data || data.length === 0) {
      container.innerHTML = '<div style="padding:40px;color:#64748b;background:#0f172a;min-height:600px;">No data. Please select: dashboard_title, explore_name, view_name, sql_table_fields</div>';
      done();
      return;
    }
    
    var fields = queryResponse.fields.dimension_like.map(function(f) { return f.name; });
    console.log('Field Search Viz - Available fields:', fields);
    ...
    var fieldsField = fields.find(function(f) { return f.toLowerCase().indexOf('sql_table_fields') !== -1 || f.toLowerCase().indexOf('falm_sql_table_fields') !== -1 || (f.toLowerCase().indexOf('fields') !== -1 && f.toLowerCase().indexOf('sql') !== -1); });
    ...
    console.log('Field Search Viz - Mapped fields:', { dashField: dashField, expField: expField, viewField: viewField, fieldsField: fieldsField });
    
    if (!dashField || !expField || !viewField || !fieldsField) {
      container.innerHTML = '<div style="padding:40px;color:#ef4444;background:#0f172a;min-height:600px;">Missing required fields.<br><br>Need: dashboard_title, explore_name, view_name, sql_table_fields<br><br>Found: ' + fields.join(', ') + '<br><br>Mapped: dashField=' + dashField + ', expField=' + expField + ', viewField=' + viewField + ', fieldsField=' + fieldsField + '</div>';
      done();
      return;
    }
    ...
    var allRows = data.map(function(row) {
      var fieldsValue = row[fieldsField] ? row[fieldsField].value : '';
      return {
        ...
        fields: fieldsValue ? fieldsValue.split('|').filter(function(f) { return f.trim(); }) : []
      };
    });
    
    console.log('Field Search Viz - Parsed rows:', allRows.length);
    console.log('Field Search Viz - Sample row:', allRows[0]);
    ...
    function render() {
      console.log('Field Search Viz - Rendering, searchTerm:', searchTerm);
      ...
      if (!searchTerm.trim()) {
        resultsHtml = '<div style="text-align:center;padding:60px 40px;">'+
          ...
          '<div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;">'+
            ['revenue','account','spend','sales','campaign'].map(function(t) {
        ...
        resultsHtml = '<div style="text-align:center;padding:60px 40px;">'+
          ...
          '<div style="color:#64748b;font-size:12px;margin-top:8px;">Try searching for different terms</div>'+
        '</div>';
      } else {
        Object.keys(grouped).forEach(function(groupKey) {
          ...
          rows.forEach(function(row, idx) {
            ...
            resultsHtml += '<div class="result-row" data-row-id="'+rowId+'" style="border-bottom:1px solid #1e293b;">'+
              '<div class="row-header" style="display:flex;align-items:center;gap:12px;padding:14px 16px;cursor:pointer;background:transparent;">'+
                '<span style="color:#64748b;">'+(isExpanded ? icons.chevronDown : icons.chevronRight)+'</span>'+
          ...
          '<div style="padding:20px 24px;border-bottom:1px solid #1e293b;">'+
            '<div style="display:flex;gap:12px;align-items:center;">'+
              '<div style="flex:1;position:relative;">'+
                ...
                '<input id="searchInput" type="text" value="'+searchTerm+'" placeholder="Search for a field name..." style="width:100%;box-sizing:border-box;padding:12px 14px 12px 44px;background:#0f172a;border:1px solid #334155;border-radius:8px;color:#e2e8f0;font-size:14px;outline:none;"/>'+
      ...
      var clearBtn = container.querySelector('#clearSearch');
      if (clearBtn) {
      ...
      var groupSelect = container.querySelector('#groupSelect');
      if (groupSelect) {
