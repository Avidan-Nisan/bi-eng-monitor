    ...
    updateAsync: function(data, element, config, queryResponse, details, done) {
      ...
      if(mode==='airflow'){
        ...
        for(var tbi=0;tbi<dagRows.length;tbi++){
          ...
          tbl+='<tr style="border-bottom:1px solid rgba(30,41,59,0.35)"><td style="padding:8px 10px 8px 0;vertical-align:middle"><div style="font-weight:600;color:#e2e8f0">'+escAf(dgr.n)+'</div></td><td style="padding:8px 10px 8px 0"><span style="display:inline-block;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;'+srBadgeStyle(dgr.sr)+'">'+dgr.sr+'%</span></td><td style="padding:8px 10px 8px 0;color:#cbd5e1">'+durStr+'</td><td style="padding:8px 10px 8px 0;color:'+(dgr.rr>35?'#f85149':dgr.rr>15?'#d29922':'#3fb950')+'">'+dgr.rr+'%</td><td style="padding:8px 10px 8px 0;color:'+qdCol+'">'+qdStr+'</td><td style="padding:8px 10px 8px 0">'+sparkHtml(dgr.sp,dgr.sr)+'</td></tr>';
        }
        ...
        if(prevD)sc1Num+='<div style="font-size:11px;color:#94a3b8;margin-top:4px">'+(dDelta>=0?'+':'')+dDelta+' vs prior day in data</div>';
        var sc2Inner='<div style="font-size:26px;font-weight:600;color:'+(succRate>=90?'#3fb950':succRate>=70?'#d29922':'#f85149')+'">'+succRate+'%</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">7d baseline '+baselineSR+'%</div>';
        ...
        if(isFinite(baselineQmin))sc4Inner+='<div style="font-size:11px;color:#94a3b8;margin-top:4px">7d avg queue '+baselineQmin+' min</div>';

        var wrap=navBar()+'<div class="lx-body" style="background:#0f172a;display:flex;flex-direction:column;min-height:0;overflow:auto"><div style="padding:16px 20px 8px;display:flex;justify-content:space-between;align-items:flex-start"><div><div style="font-size:16px;font-weight:600;color:#e2e8f0">Airflow DAG performance</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">One row per DAG run · latest calendar day in data: '+escAf(maxD)+'</div></div></div>';
        wrap+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:0 20px 16px">';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">Runs (latest day)</div>'+sc1Num+'</div>';
        ...
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">Failed runs (day)</div>'+sc3Num+'</div>';
        ...
        wrap+='<div style="display:grid;grid-template-columns:1.5fr 1fr;gap:12px;margin:0 20px 16px">';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">Run outcome trend</div><svg width="100%" height="'+hCh+'" viewBox="0 0 '+wCh+' '+hCh+'" preserveAspectRatio="none" style="display:block">'+polyLine(trendSucc,'#3fb950',false)+polyLine(trendFail,'#f85149',true)+polyLine(trendRetr,'#d29922',true)+'</svg><div style="display:flex;gap:14px;margin-top:8px;font-size:10px;color:#94a3b8"><span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:3px;background:#3fb950;border-radius:2px"></span>Successes</span><span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:3px;background:#f85149;border-radius:2px"></span>Failures</span><span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:3px;background:#d29922;border-radius:2px"></span>Retries (sum)</span></div></div>';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">Most failing DAGs (7d)</div>'+(failBars||'<div style="color:#64748b;font-size:12px">No failed runs in window</div>')+'</div></div>';
        wrap+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:0 20px 16px">';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">Retry rate (retries / tasks, 7d)</div>'+retryBars+'</div>';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">Avg queue delay by hour (UTC)</div><div style="display:flex;align-items:flex-end;gap:2px;height:88px">'+queueBars+'</div></div></div>';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px;margin:0 20px 16px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">Run starts by hour · click a bar</div><div style="display:flex;align-items:flex-end;gap:2px;height:88px" id="lx-af-hourly">'+hourlyBars+'</div><div id="lx-af-drill" style="margin-top:12px;padding-top:12px;border-top:1px solid #1e293b;font-size:12px;color:#94a3b8">Click a bar to list DAGs with runs in that hour</div></div>';
        wrap+='<div style="background:#131b2e;border:1px solid #1e293b;border-radius:8px;padding:14px 16px;margin:0 20px 20px"><div style="font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px">DAG overview (7d, lowest success first)</div><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">DAG</th><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">Success</th><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">Avg duration</th><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">Retry / tasks</th><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">Queue</th><th style="text-align:left;color:#94a3b8;font-weight:500;padding:0 10px 8px 0;border-bottom:1px solid #1e293b">7d trend</th></tr></thead><tbody>'+tbl+'</tbody></table></div></div>';
